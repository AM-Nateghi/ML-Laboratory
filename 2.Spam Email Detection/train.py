...
def load_dataset():
    ...
    # Use email and label columns
    data = data[['email', 'label']].copy()
    
    # Convert string labels to numeric if needed
    if data['label'].dtype == 'object':
        data['label'] = data['label'].map({'ham': 0, 'spam': 1})
    
    print(f"Dataset loaded: {data.shape[0]} emails")
...
def preprocess_dataset(data):
    ...
    data["cleaned_text"] = data["email"].progress_apply(preprocess_text_optimized)
...
def main():
    ...
    # Save best model
    best_model_name, best_f1_score = save_best_model(results, trained_models, tfidf_vectorizer)
